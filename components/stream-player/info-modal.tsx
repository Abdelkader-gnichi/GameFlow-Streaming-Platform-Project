...
export function InfoModal({ initialName, initialThumbnailUrl }: InfoModalProps){
    ...
    function onSubmit(e: React.FormEvent<HTMLFormElement>){
        ...
        startTransition(() => {
            ...
            .then(() => {
                toast.success(`Stream name updated successfully to ${name}`)
                closeRef.current?.click();
            })
            .catch(() => toast.error(`Something went wrong during updating the stream name from ${initialName} to ${name}`));
        });
    }
    ...
    function onRemove(){
        startTransition(() => {
            ...
            .catch(() => toast.error(`Something went wrong during thumbnail removing`));
        });
    }
                    ...
                    <div className="space-y-2">
                        ...
                        {thumbnailUrl ? (
                            ...
                            <div className="rounded-xl border outline-dashed outline-muted">
                                <UploadDropzone endpoint="thumbnailUploader" appearance={{ label: {color: "#FFFFFF" }, allowedContent:{ color: "#FFFFFF" }}} onClientUploadComplete={(res) => {
